...
for col_idx, col_def in enumerate(COLUMNS):
    ...
    with kanban_cols[col_idx]:
        # Column header
        st.markdown(
            f"""<div class="kanban-col-header" style="color:{col_def['color']};border-color:{col_def['border_color']};">
                {col_def['label']}
        ...
        if not items:
            st.markdown(
                f"""<div style="text-align:center;padding:2rem 0.5rem;color:#475569;font-size:0.8rem;">
                    {col_def['empty_msg']}
                </div>""",
                unsafe_allow_html=True,
            )
